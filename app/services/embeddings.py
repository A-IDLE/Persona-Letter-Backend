...
def embed_letter(letter: Letter):
    ...
    embeddings_model = OpenAIEmbeddings(
        model=EMBEDDING_MODEL)  # 사용할 embedding 모델
...
def text_to_vector(texts):
    try:
        ...
        print(
            f"Text to vector function completed for {len(embeddings['data'])} documents.")
        return embeddings
...
def embed_letter_pinecone_test(letter: Letter):
    ...
    response = advanced_preprocessing_by_llm(letter_content)
